...
const inputClass = "mt-2 w-full rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-white outline-none transition placeholder:text-slate-600 hover:border-white/20 focus:border-gold/50 focus:ring-2 focus:ring-gold/20 disabled:opacity-50 disabled:cursor-not-allowed";
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <form data-testid="settings-form-card" action={updateProfile} className={cardClass}>
        ...
        <div className="mt-6 space-y-4">
          ...
          {profileMessage && <p className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-300">{profileMessage}</p>}
          <button className="premium-button mt-2 w-full rounded-2xl bg-gold px-4 py-3 font-semibold text-slate-950 shadow-lg shadow-gold/10 transition hover:bg-goldLight">Save profile</button>
        </div>
      </form>
      ...
      <form data-testid="settings-form-card" action={updatePassword} className={cardClass}>
        ...
        <div className="mt-6 space-y-4">
          ...
          {passwordMessage && <p className="rounded-2xl border border-gold/20 bg-gold/10 px-4 py-3 text-sm text-goldLight">{passwordMessage}</p>}
          <button className="premium-button mt-2 w-full rounded-2xl bg-gold px-4 py-3 font-semibold text-slate-950 shadow-lg shadow-gold/10 transition hover:bg-goldLight">Update password</button>
        </div>
      </form>
    </div>
  );
